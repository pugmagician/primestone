...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PrimeStone address (e.g. %1)").arg("XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg"));
#endif
...
{
    // return if URI is not valid or is no primestone: URI
    if(!uri.isValid() || uri.scheme() != QString("primestone"))
        return false;
...
{
    // Convert primestone:// to primestone:
    //
    //    Cannot handle this later, because primestone:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("primestone://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "primestone:");
    }
...
{
    QString ret = QString("primestone:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open primestone.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PrimeStone.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PrimeStone (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("PrimeStone (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "PrimeStone*.lnk"
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "primestonecore.desktop";
    return GetAutostartDir() / strprintf("primestonecore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a primestonecore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=PrimeStone\n";
        else
            optionFile << strprintf("Name=PrimeStone (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the PrimeStone app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add PrimeStone app to startup item list
