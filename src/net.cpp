...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "PrimeStone " + FormatFullVersion();
...
#else
    threadSocketHandler = boost::thread(&TraceThread<boost::function<void()> >, "net", boost::function<void()>(boost::bind(&CConnman::ThreadSocketHandler, this)));
#endif
...
#else
	threadDNSAddressSeed = boost::thread(&TraceThread<boost::function<void()> >, "dnsseed", boost::function<void()>(boost::bind(&CConnman::ThreadDNSAddressSeed, this)));
#endif
    }
...
#else
    threadOpenAddedConnections = boost::thread(&TraceThread<boost::function<void()> >, "addcon", boost::function<void()>(boost::bind(&CConnman::ThreadOpenAddedConnections, this)));
    // Initiate outbound connections
    threadOpenConnections = boost::thread(&TraceThread<boost::function<void()> >, "opencon", boost::function<void()>(boost::bind(&CConnman::ThreadOpenConnections, this)));

    // Initiate masternode connections
    threadMnbRequestConnections = boost::thread(&TraceThread<boost::function<void()> >, "mnbcon", boost::function<void()>(boost::bind(&CConnman::ThreadMnbRequestConnections, this)));

    // Process messages
    threadMessageHandler = boost::thread(&TraceThread<boost::function<void()> >, "msghand", boost::function<void()>(boost::bind(&CConnman::ThreadMessageHandler, this)));
